...
export default function TagSelector({setShowTagsPage, selectedTags, toggleTag, handleSubmit, prof_id}: TypeTagSelector) {
    ...
    <div className="p-4 space-y-8 mb-2 mt-2">
      <div className="w-full max-w-md p-6 bg-[#111] rounded-lg shadow-xl">
        <div className="space-y-4">
          ...
          <div className="mt-4">
            <h4 className="text-lg font-semibold mb-2 text-white">Selected Tags: (Max 3)</h4>
            <div className="flex flex-wrap gap-2">
              {selectedTags.map((tag) => (
                <Badge key={tag} variant="outline" className="bg-white text-black font-semibold">
                  {formatTagName(tag)}
                </Badge>
              ))}
            </div>
        ...
        <DrawerFooter>
              ...
              <Button onClick={() => setShowTagsPage(false)} variant="outline"> <BiLeftArrow /> Back</Button>
            </DrawerFooter>
      </div>
    </div>
  )
