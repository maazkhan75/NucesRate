    ...
    export default function ReviewCards({ reviews,student_email, prof_id }: { reviews: ReviewType[] , student_email: string|undefined, prof_id:number}) {
        ...
        const distinctTags = Array.from(new Set(reviews.flatMap(review => review.tag_names)));

        useEffect(() => {
            let filtered = reviews;

            if (selectedTag) {
                filtered = filtered.filter(review => review.tag_names.includes(selectedTag));
            }
                    ...
                    filteredReviews.map((review) => (
                        <div key={review.review_id} className="bg-muted rounded-3xl p-4 mb-4">
                            <div className="flex items-center space-x-2 mb-2 justify-between">
                                <p className="text-sm text-muted-foreground">{review.student_email}  <Badge className='ml-3'>{review.review_status}</Badge></p>
                                {review.is_user_review && <AddReviewButton prof_id={prof_id} p_rating={review.rating} p_comment={review.comment} p_string={review.tag_names} />}
                            </div>
                            ...
                            <div className='flex flex-col  sm:justify-between sm:items-center sm:flex-row'>
                                ...
                                <div className="flex space-x-2">
                                    {
                                        review.tag_names && review.tag_names.map((tag, index) => {
                                            return (
                                                <Badge key={tag} className="ml-2 h-6 hover:pointer cursor-pointer">
                                                    <p>{tag}</p>
                                                </Badge>
                                            );
                                        })
