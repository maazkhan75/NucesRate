...
export default async function ProfessorsCards({page_number, url} : {page_number: number, url: ProfessorsPageUrl}) {
    ...
    const { data, error } = await supabase.rpc('get_professors_with_details', {
      ...
      }).range(page_number * 10, (page_number + 1) * 10 - 1);

    if (error) throw Error('DB Error!');
        ...
        <div className={styles.cover}>
            ...
            {professors.map(professor => {
                return(
                    ...
                    <Image className={styles.prof_image} height={300} width={150} src={professor.img_src || "/pictures/john_doe.jpg"} alt="Professor Image" />
                    </div>
                    </Card>
                );
            })}
