...
export default async function ProfessorsCards({
  page_number,
  url,
}: {
  page_number: number;
          ...
          {professors.map((professor) => {
            return (
              ...
              >
                  ...
                  <div className={styles.partition}>
                    <div className={styles.reviews_buttons}>
                      <Button className={styles.reviews_rating}>
                        {Array.from({
                          length: Math.floor(professor.average_rating),
                        }).map((_, index) => (
                          <FaStar
                            style={{ margin: "0.1rem" }}
                            key={index}
                        ...
                        {Array.from({
                          length: 5 - Math.floor(professor.average_rating),
                        }).map((_, index) => (
                          <FaStar
                            key={index}
                            style={{ margin: "0.1rem" }}
                    ...
                    <div className={styles.image_container}>
                      <Image
                        className={styles.prof_image}
                        height={150}
                        width={100}
                        src={
                          professor.img_src || "/assets/imgs/female-avatar.png"
                        }
                        alt="Professor Image"
                      />
                    </div>
                ...
                <CardFooter>
                  {professor.tag_names &&
                    professor.tag_names.map((tag) => {
                      return (
                        tag && (
                          <Badge
                            key={tag}
                            className="ml-2 hover:pointer cursor-pointer"
                          >
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger>
